...
export function clientPermissions(role:SessionRole):string[]{ if(role==="SUPER_ADMIN") return superAdmin; if(role==="ADMIN") return admin; if(role==="SAFETY_MANAGER"||role==="TRAINING_MANAGER") return safety; if(role==="SUPERVISOR") return supervisor; return []; }
export function isAdminRole(role:SessionRole){ return clientPermissions(role).includes("admin.access"); }
