...
import { visualTrainingModuleFor, type VisualTrainingSlug } from "@/data/visual-training";
import { completeTrainingAttempt, startTrainingAttempt, type RuntimeAttempt } from "@/lib/training-runtime";
import styles from "./VisualTraining.module.css";

export default function VisualTrainingQuiz({ moduleSlug }: { moduleSlug: VisualTrainingSlug }) {
  ...
  async function submit() {
    ...
    try {
      await completeTrainingAttempt(
        activeAttempt.id,
        activeModule.quiz.map((question) => ({ questionCode: question.code, optionIndex: answers[question.code] })),
      );
        ...
        <div className={styles.body}>
          ...
          <div className={styles.quizGrid}>
            {activeModule.quiz.map((question, questionIndex) => (
              <fieldset key={question.code} className={styles.quizFieldset}>
                ...
                <div className={styles.quizOptions}>
                  {question.options.map((option, optionIndex) => (
                    <label className={styles.quizOption} key={option}>
                      <input type="radio" name={question.code} checked={answers[question.code] === optionIndex} onChange={() => setAnswers((current) => ({ ...current, [question.code]: optionIndex }))} />
                      <span>{option}</span>
                    </label>
                  ))}
                </div>
